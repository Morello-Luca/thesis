...
// INIT PARAM
    ...
    // Stiffness / damping
          ...
          // Setting
                  cartesian_stiffness_.topLeftCorner(3, 3) << 1000*Eigen::Matrix3d::Identity();
                  cartesian_stiffness_.bottomRightCorner(3, 3) << 150*Eigen::Matrix3d::Identity();
                  cartesian_stiffness_target_ = cartesian_stiffness_;
                  
                  cartesian_damping_.topLeftCorner(3, 3) = 2.0 * sqrt (1000)*Eigen::Matrix3d::Identity();
                  cartesian_damping_.bottomRightCorner(3, 3) = 2.0 * sqrt(150)*Eigen::Matrix3d::Identity();
    ...
    // Position Vector 
          ee_p_cr = Eigen::Vector3d(0.0, 0.0, 0.5);
          double ee_f_d {0}; 
...
// compute initial velocity with jacobian and set x_attractor and q_d_nullspace to initial configuration
    // set equilibrium point to current state
        Eigen::Vector3d correction(-0.4, 1.0, 0.0);
        position_d_ = initial_transform.translation() +correction + initial_transform.linear()*ee_p_cr;
        orientation_d_ = Eigen::Quaterniond(initial_transform.linear());
  
        position_d_target_ = initial_transform.translation()+correction + initial_transform.linear()*ee_p_cr;;
...
// CARTESIAN CONTROLLER
    // compute error to desired pose
        // position error
            Eigen::Vector3d correction(-0.4, 1.0, 0.0);
            ActualPosition = position + correction +transform.linear()*ee_p_cr;
    ...
    // Stiffness variation
        ...
        // Update Stiffness 
              cartesian_stiffness_(2,2)  = impendance_rho*1000;
...
// Force Control
    ...
    // Rho Calculation
        if (fabs(ee_x_z) >= 2*delta_force_ ) {                                               // Determino la rho force  
            ...
            //cout<<"errore lungo z panda1"<<fabs(ee_x_z)<<endl;
