...
std::string matrix_name = "/home/luca/catkin_ws/src/panda_controllers/src/matrice_pc.csv";
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pub_pose = node.advertise<geometry_msgs::PoseStamped>("/panda_1/panda1/equilibrium_pose", 1); //DA SISTEMARE SIA PER IL NOME CHE PER IL TIPO DI MSG
    ...
    ros::Subscriber robot_pose_sub = node.subscribe("/panda_1/panda1/franka_ee_pose", 1, robotPoseCallback);
    ...
    while (ros::ok())
    {
        ...
        if (initial_pose_init)
        {
            ...
            for (int i = 0; i < NUM_COLS; i++)
            {
                ...
                pub_pose.publish(actual_posestamped_msg);
