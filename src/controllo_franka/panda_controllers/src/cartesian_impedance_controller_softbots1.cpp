...
// SUBSCRIBERS
  ...
  // Franka pose subscriber
          sub_equilibrium_pose_ = node_handle.subscribe("equilibrium_pose", 1, &CartesianImpedanceControllerSoftbots::equilibriumPoseCallback, this,ros::TransportHints().reliable().tcpNoDelay());
  
  // Desired ee force subcriber 
          sub_desired_force_ = node_handle.subscribe("desired_force", 1, &CartesianImpedanceControllerSoftbots::desiredforceCallback, this,ros::TransportHints().reliable().tcpNoDelay());
  ...
  // Disturb force
          ...
          sub_disturb_force_ = node_handle.subscribe("disturb_force", 1, &CartesianImpedanceControllerSoftbots::disturbforceCallback, this,ros::TransportHints().reliable().tcpNoDelay());
...
// Force Control
    ...
    // Rho Calculation
        if (fabs(ee_x_z) >= 2*delta_force_ ) {                                               // Determino la rho force  
            ...
            cout<<"errore lungo z panda1 "<<endl<<ee_x_z<<endl;
    ...
    // tau dist
      ...
      //<<"tau frc panda1"<<endl<<tau_frc<<endl;
...
void CartesianImpedanceControllerSoftbots::desiredforceCallback(
  ...
  void CartesianImpedanceControllerSoftbots::disturbforceCallback(
