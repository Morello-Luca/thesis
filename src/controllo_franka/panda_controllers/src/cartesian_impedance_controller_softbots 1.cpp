...
void CartesianImpedanceControllerSoftbots::starting(const ros::Time& /*time*/) {
  ...
  Eigen::Vector3d ee_p_cr(0.0, 0.0, 0.3);
  position_d_ = initial_transform.translation()  + initial_transform.linear()*ee_p_cr;
  orientation_d_ = Eigen::Quaterniond(initial_transform.linear());
  position_d_target_ = initial_transform.translation() + initial_transform.linear()*ee_p_cr;;
...
void CartesianImpedanceControllerSoftbots::update(const ros::Time& /*time*/,
  ...
  // JACOBIAN CENTER ROTATION OBJECT 
        ...
        Eigen::Vector3d ee_p_cr(0.0, 0.0, 0.3);   // Vettore posizione end-effector centro di rotazione oggetto ASSUNTO NOTO PER IL MOMENTO
  ...
  ActualPosition = position + transform.linear()*ee_p_cr;
  ...
  tau_task << J.transpose() * (-cartesian_stiffness_ * error - cartesian_damping_ * (jacobian * dq));
  ...
  //tau_d << tau_task + coriolis;
          ...
          cout<<"posizione desiderata" <<endl<< position_d_<<endl;
