...
// INCLUDE SECTION
            #include <franka_example_controllers/cartesian_impedance_example_controller2.h>
            #include <cmath>
            #include <memory>
            #include <iostream>
            ...
            #include <franka_example_controllers/get_CoriolisMatrix2.h>
...
bool CartesianImpedanceExampleController2::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {

// SUBSCRIBERS
  // Franka pose subscriber
            sub_equilibrium_pose_ = node_handle.subscribe("equilibrium_pose", 1, &CartesianImpedanceExampleController2::equilibriumPoseCallback, this,ros::TransportHints().reliable().tcpNoDelay());
            sub_second_arm_ = node_handle.subscribe("position_len", 1, &CartesianImpedanceExampleController2::SecondArmCallback, this,ros::TransportHints().reliable().tcpNoDelay());
  // Desired Contact force subscriber
            sub_desired_force_ = node_handle.subscribe("desired_force", 1, &CartesianImpedanceExampleController2::desiredforceCallback, this,ros::TransportHints().reliable().tcpNoDelay());
            sub_stima_massa = node_handle.subscribe("/combined_panda/cartesian_impedance_example_controller1/stima_forza", 1, &CartesianImpedanceExampleController2::MassEstimateCallback, this,ros::TransportHints().reliable().tcpNoDelay());
// PUBLISHERS
  // Pose ee
  ...
  // Force control check          
            pub_force_error = node_handle.advertise<geometry_msgs::Vector3Stamped>("force_control_errors", 1);
            		pub_len_obj = node_handle.advertise<geometry_msgs::Vector3Stamped>("obj_len", 1);	
...
// HANDLERS 
  ...
  dynamic_server_compliance_param_->setCallback(boost::bind(&CartesianImpedanceExampleController2::complianceParamCallback, this, _1, _2));
...
void CartesianImpedanceExampleController2::starting(const ros::Time& /*time*/) {
...
void CartesianImpedanceExampleController2::update(const ros::Time& /*time*/,
  ...
  // CARTESIAN CONTROLLER 
              ...
              Eigen::Matrix<double, 6, 1> error;
                      ...
                      Position_len = position+correction;
                              //std::cout<<"third"<<std::endl<<Position_len<<std::endl;
  ...
  // Force Control
              ...
              if (!force_tax){ 
                        ...
                        ee_f_ext_z = force_measured(1);
              }


              // Force error
                            double ee_f_ext_tilde = ee_f_d-fabs(ee_f_ext_z) ;    // Force error
  ...
  // PUBLISHERS 
        ...
        // Pub ee pose
              ...
              geometry_msgs::Vector3Stamped msg_len_3d;   
        	    msg_len_3d.vector.x    = len_object(0);  
        	    msg_len_3d.vector.y    = len_object(1);
        	    msg_len_3d.vector.z    = len_object(2);
        	    pub_len_obj.publish(msg_len_3d);
        ...
        // force check
              ...
              msg_force_error.vector.z    = EstimateMass;   // errore accumulato
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceExampleController2::saturateTorqueRate(
...
void CartesianImpedanceExampleController2::complianceParamCallback(
...
void CartesianImpedanceExampleController2::equilibriumPoseCallback(
...
void CartesianImpedanceExampleController2::SecondArmCallback(const geometry_msgs::Vector3StampedConstPtr& msg) {
  			position_second_arm << msg->vector.x, 
  						                msg->vector.y, 
  						                msg->vector.z;  	
        
        //std::cout<<"position_first"<<std::endl<<position_second_arm<<std::endl;
        //std::cout<<"position_second"<<std::endl<<Position_len<<std::endl;


  			len_object = position_second_arm-Position_len;
  			len_object = len_object.cwiseAbs();  			}

void CartesianImpedanceExampleController2::MassEstimateCallback(const geometry_msgs::WrenchStamped& msg){
      			Eigen::VectorXd second_arm_estimate(3);
      			double EstimateMass_temp;
      			EstimateMass_old = EstimateMass;
      			second_arm_estimate << 	msg.wrench.force.x,
							                      msg.wrench.force.y,
							                      msg.wrench.force.z; 	
							
      			double EstimateMass_newton = fabs(second_arm_estimate(2) + stima(2));
      			EstimateMass_temp = EstimateMass_newton/9.80665;
      			EstimateMass = (EstimateMass_temp+EstimateMass_old)/2;}




void CartesianImpedanceExampleController2::desiredforceCallback(
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianImpedanceExampleController2,
