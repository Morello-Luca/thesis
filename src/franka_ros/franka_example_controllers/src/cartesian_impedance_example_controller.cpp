...
// SUBSCRIBERS
  // Franka pose subscriber
            sub_equilibrium_pose_ = node_handle.subscribe("equilibrium_pose", 1, &CartesianImpedanceExampleController::equilibriumPoseCallback, this,ros::TransportHints().reliable().tcpNoDelay());
  // Desired Contact force subscriber
            sub_desired_force_ = node_handle.subscribe("desired_force", 1, &CartesianImpedanceExampleController::desiredforceCallback, this,ros::TransportHints().reliable().tcpNoDelay());

// PUBLISHERS
  // Pose ee
            pub_endeffector_pose_ = node_handle.advertise<geometry_msgs::PoseStamped>("franka_ee_pose", 1);
            		pub_position_3d = node_handle.advertise<geometry_msgs::Vector3Stamped>("/combined_panda/cartesian_impedance_example_controller2/position_len", 1);		
...
void CartesianImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  // Force Control
              ...
              if (!force_tax){ 
                        ee_f_ext_z = 0;
                        std::cout<<"ee_f_ext_z"<<std::endl<<ee_f_ext_z<<std::endl;
              } else{
  ...
  // OUTPUT TAU         
              if (!force_tax){ 
                        tau_d << tau_imp;
                        std::cout<<"tau_d = tau_imp"<<std::endl;
              } else{
                        tau_d << tau_imp+tau_frc;
                        std::cout<<"tau_d = tau_imp +tau_frc"<<std::endl;
              }
  ...
  // PUBLISHERS 
        ...
        // Pub ee pose
             ...
             geometry_msgs::Vector3Stamped msg_position_3d;   
        	    msg_position_3d.vector.x    = (position(0) + correction(0));  
        	    msg_position_3d.vector.y    = (position(1) + correction(1));
        	    msg_position_3d.vector.z    = (position(2) + correction(2));
        	    pub_position_3d.publish(msg_position_3d);
...
void CartesianImpedanceExampleController::desiredforceCallback(
