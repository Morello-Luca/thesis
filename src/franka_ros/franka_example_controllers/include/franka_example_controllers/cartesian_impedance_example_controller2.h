...
class CartesianImpedanceExampleController2 : public controller_interface::MultiInterfaceController<
 ...
 private:
  ...
  // CARTESIAN PARAM
  	...
  	Eigen::Vector3d Position_len;
  // FORCE PARAM
  	double force_error_integral;
  	double ee_f_d;
  	double ee_f_d_target; 
  	double kp_ {3.0};          // Proportional gain
...
// PUBLISHERS
	  ...
	  ros::Publisher pub_force_error;
		...
		ros::Publisher pub_len_obj;		
	
		ros::Subscriber sub_second_arm_;
	  	void SecondArmCallback(const geometry_msgs::Vector3StampedConstPtr& msg);
	
		Eigen::Vector3d len_object;
		Eigen::Vector3d position_second_arm;
		
	        ros::Subscriber sub_stima_massa;
	  	void MassEstimateCallback(const geometry_msgs::WrenchStamped& msg);
	  	double EstimateMass = 0;
	  	double EstimateMass_old = 0;
	  	Eigen::Matrix<double, 7, 1> tau_frc;
	  	Eigen::Matrix<double, 3, 1> force_measured_ee;
	  	Eigen::Matrix<double, 6, 1> stima;
	  	bool force_tax = false;
	  	double ee_f_ext_z = 0;
